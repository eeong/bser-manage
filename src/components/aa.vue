<template>
  <div>
    ...
    <select class="ui dropdown" v-model="rec.weapon" :class="isSelected(currentChar)">
...
export default {
  ...
  data: function() {
    return {
      currentChar:[this.rec.character,this.rec.characterSrc] || '',
      options: [
        {
          key: '6',
          text: '나딘',
  ...
  methods: {
    ...
    weaponMatch(match){
      let list ={"나딘":['활','석궁'],"레녹스":["채찍"],"로지":["권총"],"루크":["방망이"],"리다이린":["글러브","쌍절곤"],"매그너스":["망치","방망이"],"쇼우":["단검","창"],"쇼이치":["단검"],"시셀라":["투척","암기"],"실비아":["권총"],"아델라":["레이피어"],"아드리아나":["투척"],"아야":["권총","돌격소총","저격총"],"아이솔":["권총","돌격소총"],"엠마":["암기"],"유키":["양손검"],"자히르":["투척","암기"],"재키":["단검","양손검","도끼"],"캐시":["단검"],"키아라":["레이피어"],"피오라":["레이피어","양손검","창"],"하트":["기타"],"현우":["글러브","톤파"],"혜진":["암기","활"]}
      return list[match]
    },
    weaponChange(weapon){
    ...
    isSelected(){
      if(!this.currentChar) return 'disabled'
    }
  },
  computed:{
    /* recWeaponChange: function() {
      return this.weaponChange(this.recWeapon);
    } */
  },
  watch: {
  ...
  async mounted() {
    if(this.rec){
      this.optionsW = this.weaponMatch(this.rec.character || '')
    } 
  }
};
</script>
